...
interface BuildStagesTimelineProps {
  stages: Stage[]
  jobName: string
  buildNum: string
}

export function BuildStagesTimeline({ stages, jobName, buildNum }: BuildStagesTimelineProps) {
  // 辅助函数：确保日期是Date对象
            ...
            <div className="ml-4 flex-1">
              <div className="flex items-center justify-between">
                ...
                <Button variant="ghost" size="sm" asChild>
                  <Link
                    href={`/job-monitor/${encodeURIComponent(jobName)}/${buildNum}/${encodeURIComponent(stage.name)}`}
                  >
                    查看详情
                  </Link>
                </Button>
